...
void main() {
  ...
  setUp(() {
    mockDio = MockDio();
    csrfTokenManager = CsrfTokenManager(dio: mockDio);
  });

  group('CsrfTokenManager', () {
    const testToken = 'test-csrf-token-123';
    const apiPath = '/api/v1/auth/csrf';
    ...
    group('getToken', () {
      test('should fetch and cache CSRF token from API', () async {
        ...
        when(mockDio.get(apiPath)).thenAnswer((_) async => response);
        ...
        verify(mockDio.get(apiPath)).called(1);
      });
      ...
      test('should refresh token if cache is expired', () async {
        ...
        when(mockDio.get(apiPath)).thenAnswer((_) async => response1);
        ...
        verify(mockDio.get(apiPath)).called(2);
      });

      test('should throw ServerException on API error', () async {
        // Arrange
        when(mockDio.get(apiPath)).thenThrow(
          DioException(
            requestOptions: RequestOptions(path: apiPath),
            response: Response(
              statusCode: 500,
